...
# C6 (점포 데이터) 전처리
df_c6 = c6_df.copy()
if not df_c6.empty and all(col in df_c6.columns for col in ["점포명", "위도", "경도"]):
...
dc_dict = {}
if not df_c8.empty:
    center_col = next((c for c in ["센터명", "물류센터", "DC명"] if c in df_c8.columns), None)
...
def calculate_kpi(c1, c2, c3, c4, c5, c7, store_name=None):
    ...
    for w in selected_weeks:
        ...
        sum_c1 = pd.to_numeric(c1_sub[col1], errors="coerce").sum() if (col1 and col1 in c1_sub.columns and not c1_sub.empty) else np.nan
        sum_c2 = pd.to_numeric(c2_sub[col2], errors="coerce").sum() if (col2 and col2 in c2_sub.columns and not c2_sub.empty) else np.nan
        sum_c3 = pd.to_numeric(c3_sub[col3], errors="coerce").sum() if (col3 and col3 in c3_sub.columns and not c3_sub.empty) else np.nan
        sum_c4 = pd.to_numeric(c4_sub[col4], errors="coerce").sum() if (col4 and col4 in c4_sub.columns and not c4_sub.empty) else np.nan
        sum_c5 = pd.to_numeric(c5_sub[col5], errors="coerce").sum() if (col5 and col5 in c5_sub.columns and not c5_sub.empty) else np.nan
        sum_c7 = pd.to_numeric(c7_sub[col7], errors="coerce").sum() if (col7 and col7 in c7_sub.columns and not c7_sub.empty) else np.nan
...
def get_top_5_voc_stores(c4_df, weeks):
    ...
    store_col = next((col for col in ["점포명", "점포"] if col in c4_df.columns), None)
    if not store_col:
        return pd.DataFrame(columns=["순위", "점포명", "총 VOC 건수"])

    week_cols = [get_week_col_name(c4_df, w) for w in weeks if get_week_col_name(c4_df, w)]
    ...
    grouped = temp_df.groupby(store_col)["총_VOC"].sum().reset_index().sort_values(by="총_VOC", ascending=False)

    top_5 = grouped.head(5).reset_index(drop=True)
    top_5.index = top_5.index + 1
    top_5 = top_5.reset_index().rename(columns={"index": "순위", store_col: "점포명", "총_VOC": "총 VOC 건수"})
...
df_store_kpi = calculate_kpi(c1_df, c2_df, c3_df, c4_df, c5_df, c7_df, selected_store_name)
...
with map_col:
    ...
    for dc_name, coords in dc_dict.items():
        ...
        folium.map.Marker(
            location=[dc_lat, dc_lng],
            icon=folium.DivIcon(icon_size=(0, 0), icon_anchor=(-8, 10), html=label_html),
        ).add_to(m)
    ...
    if target_row is not None:
        ...
        if dc1_raw is None and len(target_row) >= 4:
            dc1_raw = target_row.iloc[3]
        if dc2_raw is None and len(target_row) >= 5:
            dc2_raw = target_row.iloc[4]
            ...
            if not df_top5_voc.empty:
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "총 VOC 건수": st.column_config.NumberColumn("총 VOC 건수", format="%d 건", alignment="center"),
                    },
                )
            else:
